...
Skill difficulty ranges from 1-5 with the following mapping:
...
When the game master calls for a check the player making the check rolls all of their 3 character
dice. They then count the number of visible faces that apply to the scenario they are trying to do 
(the game master being the ultimate authority on what is applicable). A dice face can contribute BOTH
its attributes and skills to the check if they are applicable.
